<?php  
	...
	$sql="SELECT *, sum(cantidad) as cantidadDisponible FROM producto WHERE almacen='$almacen' AND cantidad>0 GROUP BY nombre";
...
<html>
	...
	<body>	
		<table align="center" id="lista1" border="1">
			<tr>
				<th colspan="8">AGREGAR PRODUCTOS A LA ORDEN DE SALIDA <?php  echo $orden; ?></th>
			</tr>
			<tr>
				<form method="post">
					<th colspan="8" height="35px"><input type="text" name="buscar" placeholder="Nombre del producto">       <input type="submit" name="busqueda" value="Buscar"> <a href="listaOrdenesSalida.php" style="float: right; text-decoration: none; color: green;"><label>Listo <img src="img/Check_green_icon.svg.png" style="height: 25px;"></label></a></th>
				</form>
			</tr>
			<tr>
				<th>ID PRODUCTO</th>
			...
			<?php 
				if ($resultados==0) { ?>
					...
					if (isset($_POST['buscar']) && $_POST['buscar']!='') {
						$sql1="SELECT *, sum(cantidad) as cantidadDisponible FROM producto WHERE almacen='$almacen' AND nombre LIKE '$_POST[buscar] %' OR almacen='$almacen' AND nombre LIKE '% $_POST[buscar] %' OR almacen='$almacen' AND nombre LIKE '% $_POST[buscar]' OR almacen='$almacen' AND nombre LIKE '%$_POST[buscar]%' GROUP BY nombre";
						...
						if ($resultados1==0) { ?>
							...
							while ($row1=mysqli_fetch_array($query1)) { ?>
								<form method="post" action="agregarOrdenSalida.php">
									<tr>
										...
										<td><input type="text" name="cantidad" value="1"><input type="hidden" name="orden" value="<?php echo $orden; ?>"><input type="hidden" name="almacen" value="<?php echo $almacen; ?>"><input type="hidden" name="cantidadDisponible" value="<?php echo $row1['cantidadDisponible']; ?>"></td>
						...
						while ($row=mysqli_fetch_array($query)) { ?>
							<form method="post" action="agregarOrdenSalida.php">
								<tr>
									...
									<td><input type="text" name="cantidad" value="1"><input type="hidden" name="orden" value="<?php echo $orden; ?>"><input type="hidden" name="almacen" value="<?php echo $almacen; ?>"><input type="hidden" name="cantidadDisponible" value="<?php echo $row['cantidadDisponible']; ?>"></td>
									<th><input type="submit" name="agregar" value="AGREGAR"></th>
								</tr>
							</form><?php
						}
